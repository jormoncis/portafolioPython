...
def validar_disponibilidad(video_juegos):
...
def buscar_videojuego(video_juegos,titulo_videojuego):
    for i in range(len(video_juegos)):
        if video_juegos[i]['titulo'].lower() == titulo_videojuego:
            return i
    return -1
...
def mostrar_catalogo(video_juegos):
    ...
    for i in range(len(video_juegos)):
        print(f'Nombre: {video_juegos[i]['titulo'].lower()}')
        print(f'Precio: ${video_juegos[i]['precio']}')
        print(f'Cantidad en stock: {video_juegos[i]['stock']}')
        if video_juegos[i]['disponible']:
            print('Estado: Disponible')
        else:
            print('Disponibilidad: Sin Stock')
        print('********************************************')
...
def programa_principal():
    ...
    while True:
        ...
        if opc == 1:
            ...
            titulo = input('Ingrese el titulo del videojuego: ').lower()
            ...
            if posicion != -1:
                ...
                print(f'\nEl videojuego "{titulo}" ha sido eliminado.')
            else: print('No se ha encontrado el elemento.')

        elif opc == 4:
            if video_juegos != []:
                validar_disponibilidad(video_juegos)
                print('\nDisponibilidad actualizada.')
            else:
                print('Aún no hay cantidades para actualizar.')
